...
from eval_harness.sketch.sketches.registry import get_sketch, register_sketch
...
class DMSSketch(BaseSketch):
    ...
    Wraps a ScorerSketch and evicts keys/values with scores below a given threshold.
    ...
    at ``import eval_harness.sketch``) overwrites those cache rows with fake keys k
    ...
    a forward pass) instead of a ScorerSketch field.

    Parameters
    ----------
    press : ScorerSketch or str
        ...
        ``get_sketch(press)``. The scorer is always called with ``attentions=None``
    ...
    - Prefill/decode detection keeps kvpress's ``cache_position[-1] + 1 == q_len``
      ...
      ``cache_position`` is absent (parity with ``BaseSketch._is_decoding_step``).
    ...
    def __post_init__(self):
        if isinstance(self.press, str):
            self.press = get_sketch(self.press)
        assert isinstance(self.press, ScorerSketch), "DMSSketch requires a ScorerSketch as press"
