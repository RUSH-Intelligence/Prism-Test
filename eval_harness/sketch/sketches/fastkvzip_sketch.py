...
@register_sketch("fastkvzip")
@dataclass
class FastKVzipSketch(BaseSketch):
    """
    ...
    - Prefill/decode gating uses ``BaseSketch._is_decoding_step`` instead of the
      inline ``cache_position`` comparison (framework idiom, same semantics).
    - Hook installation reuses ``BaseSketch.__call__`` (skips all non-full-
