...
@register_sketch("block")
@dataclass
class BlockSketch(BaseSketch):
    """
    ...
    ``ScorerSketch`` and keeps the ``n_kept`` best. This is not a true
    ...
    ``ScorerSketch`` is not expressible as flat config kwargs). Under the DCA
    ...
    - ``press`` field renamed ``sketch`` (framework naming; wraps a
      ``ScorerSketch`` instead of a ``ScorerPress``).
    ...
    def __post_init__(self):
        assert isinstance(self.sketch, ScorerSketch), "BlockSketch requires a ScorerSketch"
