...
@register_sketch("per_layer_compression")
@dataclass
class PerLayerCompressionSketch(BaseSketch):
    """
    ...
    ScorerSketch method. Different layers may have different importance patterns,
    ...
    **Important**: Experimental feature that only works with flash attention.
    ...
    press : ScorerSketch or str
        ...
        ratios. Must be (or resolve to) a ``ScorerSketch`` exposing
    ...
    - ``compression_ratios`` entries are not range-checked: 1.0 is accepted (the
      per-layer attribute write bypasses ``ScorerSketch.__post_init__``'s
    ...
    def __post_init__(self):
        ...
        if isinstance(self.press, str):
            self.press = get_sketch(self.press, **(self.press_kwargs or {}))
        ...
        assert isinstance(self.press, ScorerSketch), "PerLayerCompressionSketch requires a ScorerSketch as input"
