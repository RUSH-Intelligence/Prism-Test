...
SketchSpec = Union[BaseSketch, str, Sequence]


def _resolve_member(spec: SketchSpec) -> BaseSketch:
    if isinstance(spec, BaseSketch):
        return spec
    if isinstance(spec, str):
        return get_sketch(spec)
    if (
        ...
        return get_sketch(spec[0], **dict(spec[1] or {}))
    raise TypeError(
        "ComposedSketch members must be BaseSketch instances, registry names, "
        f"or (name, kwargs) pairs, got {spec!r}"
    )


@register_sketch("composed")
@dataclass
class ComposedSketch(BaseSketch):
    ...
    Parameters
    ----------
    presses : list[BaseSketch | str | (str, dict)]
