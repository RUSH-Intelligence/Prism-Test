...
@register_kv_compressor("chunk")
@dataclass
class ChunkSketch(KVCompressor):
    """
    ChunkPress: Uniform compression through independent chunk processing.

    This wrapper enhances any ScorerKVCompressor by applying compression independently
    ...
    - Per-chunk ``n_kept = max(1, int(L * (1 - ratio)))`` keeps at least one
      ...
      ratio for short/last chunks (global ``ScorerKVCompressor`` has no such floor).
    ...
    - Kept positions within each chunk are in descending-score order, not
      positional order (same as ``ScorerKVCompressor``).
