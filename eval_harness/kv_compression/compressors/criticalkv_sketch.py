...
@register_kv_compressor("criticalkv")
class CriticalKVSketch(ScorerKVCompressor):
    """
    ...
    pruning are the inherited uniform ``ScorerKVCompressor.compress``.
    ...
    def __init__(self, press: ScorerKVCompressor, epsilon: float = 1e-4, first_stage_ratio: float = 0.5):
        self.press = press
        self.epsilon = epsilon
        self.first_stage_ratio = first_stage_ratio

        assert isinstance(self.press, ScorerKVCompressor), "CriticalKVSketch requires a ScorerKVCompressor as input"
...
@register_kv_compressor("critical_adakv")
@dataclass
class CriticalAdaKVSketch(KVCompressor):
    """
