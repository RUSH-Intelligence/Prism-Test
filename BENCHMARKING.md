...
| `balancekv` ¶            | Post-prefill        | Discrepancy-theory coreset (not kvpress; port of github.com/ksheth96/BalanceKV, arXiv:2502.07861). A self-balancing random walk picks a balanced `[sink \| middle-coreset \| window]` token subset and **reweights surviving values**. Knobs: `itrs` (halvings; `compression_ratio` maps to `round(log2(1/(1-ratio)))`), `gamma`/`temp`/`beta`, `block_size`, `n_sink`, `window_size`, `seed`. |
...
¶ **`balancekv` physically prunes (real memory savings) but reweights values.** Unlike the masking-based methods it actually shrinks the cache to `n_sink + len(middle)/2**itrs + window_size` tokens. It is a coreset, not a top-k scorer, so it subclasses `KVCompressor` directly rather than `ScorerKVCompressor`. The surviving values are scaled by `weight/2**itrs`, so the cached values are no longer the model's raw values — a consequence of the approximation, not a bug. Selection runs on RoPE-rotated cached keys (kept keys stay at their original phases); as with the position-sensitive scorers, keep `attention_method: none`.
